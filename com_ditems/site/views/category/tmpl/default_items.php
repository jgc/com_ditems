...
// Check if user is allowed to add/edit based on ditems permissinos.
$canEdit = $user->authorise('core.edit', 'com_ditems.category.'.$this->category->id);
$canCreate = $user->authorise('core.create', 'com_ditems');
$canEditState = $user->authorise('core.edit.state', 'com_ditems');
...
<?php if (empty($this->items)) : ?>
	<p> <?php echo JText::_('COM_DITEMS_NO_DITEMS'); ?></p>
<?php else : ?>
...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
	...
	<fieldset class="filters btn-toolbar">
		<?php if ($this->params->get('filter_field') != 'hide') :?>
			<div class="btn-group">
				<label class="filter-search-lbl element-invisible" for="filter-search"><span class="label label-warning"><?php echo JText::_('JUNPUBLISHED'); ?></span><?php echo JText::_('COM_DITEMS_FILTER_LABEL').'&#160;'; ?></label>
				<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_DITEMS_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_DITEMS_FILTER_SEARCH_DESC'); ?>" />
			</div>
		<?php endif; ?>
	...
	<?php endif; ?>
				...
				<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
					...
					<div class="list-title">
						<?php if ($this->params->get('icons', 1) == 0) : ?>
							 <?php echo JText::_('COM_DITEMS_LINK'); ?>
						<?php elseif ($this->params->get('icons', 1) == 1) : ?>
							<?php if (!$this->params->get('link_icons')) : ?>
								<?php echo JHtml::_('image', 'system/ditem.png', JText::_('COM_DITEMS_LINK'), null, true); ?>
							<?php else: ?>
								<?php echo '<img src="'.$this->params->get('link_icons').'" alt="'.JText::_('COM_DITEMS_LINK').'" />'; ?>
							<?php endif; ?>
						<?php endif; ?>
						<?php
						...
						<?php $tagsData = $item->tags->getItemTags('com_ditems.ditem', $item->id); ?>
		...
		<?php // Code to add a link to submit a ditem. ?>
